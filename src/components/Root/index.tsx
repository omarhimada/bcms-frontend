...
import { Font, SiteContext, Configuration } from "./types";
...
const _styletron = new Styletron();
...
export default () => {
  ...
  if (error || data?.configurations === undefined || !data.configurations.length) {
    return (
      <span>
  ...
  // Override baseui default primitives
  const primitives = {
    primaryFontFamily: "Roboto",
    primaryColor: configuration.primaryColor.hex,
  };

  // Override baseui default colors
  const overrides = {
    colors: {
  ...
  return (
    <SiteContext.Provider value={configuration}>
      <StyletronProvider value={_styletron}>
        <BaseProvider theme={_baseWebTheme}>
          ...
          <Helmet>
            ...
            <link
              ...
              href={configuration.appleTouchIcon.url}
            />

            <meta
              name="theme-color"
              content={configuration.primaryColor.hex}
            />
